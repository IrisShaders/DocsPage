...
Our eyes don't percieve light linearly, but rather on a generally logarithmic scale. For example, the color that appears to your eyes as halfway between black and white actually only emits 22% of the photons that white does. Because of this, computer images (such as Minecraft's textures) are stored in the *sRGB color space*, which dedicates more precision to brighter colors. Approximately, the color value stored on the computer is the actual value of photons emitted raised to the power of 1/2.2, which is known as gamma correcting. When it comes for your monitor to display these colors to the screen, it raises the values to the power of 2.2 - the inverse of the gamma correction - to emit the correct amount of photons.

This means that a color value of 0.5 stored on a computer does look halfway between white and black. However, when working with lighting effects, we want the actual value of photons and not the human perception of the color. To achieve this, we can ourselves apply inverse gamma correction by raising the colors to the power of 2.2 before applying lighting effects. Just after we sample `colortex0` in `composite.fsh`, let's apply the following:
...
Because `colortex` buffers are designed to hold gamma corrected colors, if you write linear color to them you will lose some precision. Historically, the solution to this was to convert back to gamma space before writing to the buffer, but with modern hardware, we can simply set the buffer to store higher precision values. To do this, we can add the following code.
...
:::note[Note]
We have set the format of `colortex0` to `RGB16F`. This means we are storing R, G, and B values, as we don't care about the alpha channel for the color of the overall scene. The `16` indicates we are using 16 bits of precision, instead of the default 8. With 8 bits of precision we can only store 256 different values per channel (2^8 = 256), but with 16 bits we can store a whopping 65536! The `F` indicates that the buffer is floating point, which means that values can be higher than 1.0. This makes no difference within the scope of the tutorial, but is useful for effects like bloom. This is known as 'HDR rendering', although it is important to note that we are *not outputting HDR colors to the screen*, as Minecraft does not support this.
:::

:::caution[Warning]
Observe that the declaration for the format is in a *multi-line comment*. This is because this code does not actually need to make it onto the GPU. Instead, Iris reads it, and knows to set the format of the buffer as we defined it.
...
:::note[Note]
It's not too much of an issue where this code is actually placed in your shaderpack. A common approach for code cleanliness is to have a file included everywhere in your shader specifying this. This is explained later in the tutorial, but putting it in `composite.fsh` for the moment will suffice.
:::
