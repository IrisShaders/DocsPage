...
:::tip[VRAM Costs]
Just like your CPU, GPUs have a limited amount of RAM, know as Video RAM (VRAM). Increasing the precision of your buffers also increases their VRAM cost. While modern GPUs have more than enough VRAM to support one higher-precision buffer, it still remains best practice to fix the precision issue by instead converting your linear colors back to sRGB any time before writing them to an RGBA8 buffer. If your buffers are already higher precision, you only need to convert back to sRGB directly before outputting to the screen. Saving VRAM can become very important as you start to code more complex projects, but for the sake of simplicity, this tutorial will instead merely increase the precision.
:::
