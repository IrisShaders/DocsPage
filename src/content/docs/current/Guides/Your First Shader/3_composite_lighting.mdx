...
:::tip[VRAM Costs]
Just like your CPU, GPUs have access to a limited amount of memory, know as Video RAM (VRAM). Increasing the precision of your buffers also increases their VRAM cost. While modern GPUs have more than enough VRAM to support one higher-precision buffer, it is always important to consider how much precision you actually need. In this case, it would be more memory efficient to stick to using RGBA8 and convert colors to and from sRGB whenever writing them to the buffer, however for the purposes of this tutorial, a precision increase is simpler.
:::
