...
```
This value stores the world space offset from the player model's head position to the camera's position(ie [`cameraPosition`](/current/reference/uniforms/camera#cameraposition)`-`[`eyePosition`](/current/reference/uniforms/camera#eyeposition)).
