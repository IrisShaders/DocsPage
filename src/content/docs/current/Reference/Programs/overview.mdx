...
> **Note**: Other than some [gbuffers](/current/reference/programs/gbuffers) programs being before/after [deferred](/current/reference/programs/deferred), the order of gbuffers programs is not fixed.
...
2. **Screen size**: calculate work groups as a multiplier of screen resolution, a const in shader file [`const vec2 workGroupsRender`](/current/reference/constants/workgroupsrender)
3. **Indirect**: specify work groups in an SSBO through [`indirect.pass`](/current/reference/shadersproperties/ordering#indirectpass)
