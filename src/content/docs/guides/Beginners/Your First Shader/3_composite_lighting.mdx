...
Anyway, let's go back to our `gbuffers_terrain.fsh`. In order to apply gamma correction, we raise our colour to the power of 2.2. So, at the end of the pass:

```glsl
color.rgb = pow(color.rgb, vec3(2.2));
```

You'll notice this makes our colours look horrible. This is because we need to apply inverse gamma correction afterwards. Let's make a new shader pass called `final`. Create copies of your `composite.fsh` and `composite.vsh` files and rename them to `final.fsh` and `final.vsh`. The `final` pass is functionally identical to the `composite` passes but it always runs after the last composite pass.
...
Voila! Your colours are back to normal.
...
```
![](/src/assets/first_shader_guide/3/lightmap.png)

You can see that where there is skylight, only the green component is set. However, where there is blocklight (by the torch), the colour is yellow. Since red and green make yellow in RGB, we know that blocklight is stored in the red component and sunlight is stored in the green component.

```glsl
color.rgb = normal;
```
![](/src/assets/first_shader_guide/3/normals.png)
...
## Lighting
To apply lighting to our colour, we want to multiply it by the colour of the light hitting it. Of course, with multiple light sources, there are multiple types of light hitting it. For our shader, we will have the following types of light
- Ambient (some constant factor, so you can see in caves)
- Skylight (blue in the day, uses the lightmap)
- Blocklight (some warm colour that matches torches, uses the lightmap)
- Sun/Moonlight (yellow for the sun, white for the moon)

So, to apply our lighting, we would then do something like `color.rgb *= ambient + skylight + blocklight + sunlight`.

First, let's define some colours/brightnesses for each term. I chose the following on a whim.
```glsl
...
I have defined these before `main`, they are `const`s which means the definition can run outside of a the main function.
...
![](/src/assets/first_shader_guide/3/lighting2.png)
...
To access the depth buffer, we use `uniform sampler2D depthtex0`. In between the code which reads the colour from `colortex0` and the code which applies lighting, we can check if the pixel is the sky, and if it is, we can `return` from the `main` function.
...
![](/src/assets/first_shader_guide/3/fixedsky.png)