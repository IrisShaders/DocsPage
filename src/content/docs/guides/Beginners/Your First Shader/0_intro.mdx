...
## Prerequisites
:::caution[Warning]
**This tutorial is designed for people who already know how to code. If you do not know how to code, you will likely struggle with this tutorial.** A general understanding of how computer graphics work is useful but not necessary.
:::
...
It is recommended that you use a Minecraft launcher which supports viewing the logs in realtime. One such choice is [Prism Launcher](https://prismlauncher.org/).
...
The final type of pass is the shadow pass, which runs before all other passes. This pass renders all terrain from the perspective of the sun to a few buffers we can access later, known as the 'shadow maps'. The main shadow map stores how far away the closest thing it can see is. From this, we can check if something is further from the sun than the closest thing it can see, and if it is, it must be in shadow. We will cover this later on in the tutorial.
