...
This transforms the position of the vertex from model space to clip space. The `ftransform` function is actually deprecated but Iris patches it to the relevant modern code. For more information, see the howto on (coordinate spaces)[/how_to/coordinate-spaces].
...
![](/src/assets/first_shader_guide/monochrome.webp)
