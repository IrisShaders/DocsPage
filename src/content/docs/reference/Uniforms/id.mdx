...
Using `uniform int currentRenderedItemId;`, you can detect items and armor rendered in the level at the point of render (based on [`item.properties`](/reference/miscellaneous/item_properties)). This is similar to [`heldItemId`](/reference/uniforms/id#helditemid) and [`heldItemId2`](/reference/uniforms/id#helditemid2), except it applies to the currently rendered item/armor instead of the held item.
