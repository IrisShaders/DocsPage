...
Equivalent to [`playerMood`](/reference/uniforms/status#playermood) except when it does not reset after hitting `1.0`, remaining at that value until the player leaves a cave or otherwise causes the mood value to decrease.
