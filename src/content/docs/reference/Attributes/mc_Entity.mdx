...
The `x` component stores block ID which was assigned to the current block in the [`block.properties`](/reference/miscellaneous/block_properties) file. If an ID for the current block is not defined, a value of `-1` (newer Iris versions) or `0` (older Iris versions, Optifine) is sent.
